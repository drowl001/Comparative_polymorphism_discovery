    ...
    with open(bed_filename, 'r') as bed_file:
        for line in bed_file:
            ...
            for pileupcolumn in bamfile.pileup(chrom, start, end+1):
                ...
                if coverage >= 5:
                    for pos in range(max(start, position), min(end + 1, position + 1)):
                        coding_region_coverage[(chrom, pos)] = 1

    # Calculate total bases covered at 5X coverage
    total_bases_5x_coverage = len(coding_region_coverage)

    # Write the result to the output file
    with open(output_filename, 'w') as output_file:
        output_file.write(f"Total bases in coding region covered at 5X: {total_bases_5x_coverage} bases\n")
